...
dataGWR <- sf::read_sf("gg_GWR_Input_summer_datazones_SIMD_extra.shp") # Reading as an sf 
...
correlation_matrix <- cor(dataGWR_df[, c("mean_noise", "incnorm", "empnorm" , "hlthnorm","depratnorm" ,"pctmineth",
                                         "HouseOCrat", "HouseNCrat", "HouseRank",  "Vigintilv2", "EduRank"
                                         
)])
corrplot(correlation_matrix, method = "circle", type = "upper")

library(openxlsx)
write.xlsx(correlation_matrix, file = "C:/Users/ibk1/Desktop/results/RQ3/final variables gg/correlationmatrix.xlsx", rowNames = TRUE)
...
# Quick mapping of one of the independent variables
library(tmap)
tm_shape(dataGWR) + tm_fill("pctmineth", palette = "Reds", n = 7, style = "equal")

# Create a unique Poly_ID for use in GeoDa
dataGWR$POLY_ID <- seq_len(nrow(dataGWR))
colnames(dataGWR)

# Optional: export shapefile for use in GeoDa
# library(sf)
# st_write(dataGWR, "Lden_ScotGov_SIMD_Combined_PolyID.shp", append=FALSE)
...
variablesAsAdded <- c("depratnorm","pctmineth", "HouseOCrat", "HouseNCrat", "hlthnorm")
...
finalFormula <- mean_noise ~ pctmineth + depratnorm + HouseOCrat + HouseNCrat + hlthnorm
...
dataGWR$predictedmean_noise <- 5.079310 +1.524220  * dataGWR$pctmineth  +   0.014108 * dataGWR$HouseOCrat  +  -3.822326   * dataGWR$depratnorm      +   0.203142   * dataGWR$hlthnorm  + 0.012762   * dataGWR$HouseNCrat        
...
st_write(mapGWR,"GG_GWR_Results_rq3_final_FIVE.shp", append=FALSE)